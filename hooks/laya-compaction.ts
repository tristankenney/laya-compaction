...
export async function compactSession(
  ...
  // A local Laya server ignores the Authorization header, but JevClient and
  // buildJevRequest still want a non-empty key, so a placeholder stands in.
  // Only the hosted endpoint genuinely needs one.
  if (!config.apiKey && !config.baseUrl) {
    throw new Error('no baseUrl and no TYPESAFE_API_KEY: set one of them');
  }
